...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # at the last step take the final values
...
class RTDETRTransformer(nn.Module):
    ...
    # THIS function is in charge of preparing the input for the decoder by selecting the best features
    # It incorporates the Uncertainty-minimal Query Selection (UQS)?
    def _get_decoder_input(self,
        ...
        # implementation of the UMQS (Uncertainty-minimal Query Selection)
        ...
        #                           │
        #           ┌───────────────▼────────────────┐
        #           │   Generate Anchors & Mask      │
        #           │ (self._generate_anchors / pre) │
        #           └───────┬───────────┬────────────┘
        #                   │           │
        #                   │           │ valid_mask
        #                   │           ▼
        ...
        #         │  Encoder Heads           │
        #         │                          │
        #         │ enc_score_head → classes │
        #         │ enc_bbox_head  → bboxes  │
        #         └───────┬───────────┬──────┘
        #                 │           │
        #                 │           │+ anchors
        #                 │           ▼
        ...
        #                 ▼
        #      ┌───────────────────────────────┐
        #      │ UMQS: Select Top-K Queries    │
        #      │  topk over class confidences  │
        #      └───────┬───────────────────────┘
        #              │ indices
        #              ▼
        #    ┌─────────────────────────────┐
        #    │ Gather Top-K from encoder   │
        #    │  - bboxes (sigmoid)         │
        #    │  - logits                   │
        #    │  - memory features          │
        #    └───────┬─────────────────────┘
    ...
    def forward(self, feats, targets=None):
        ...
        out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], \
               'enc_pred_logits': enc_topk_logits, 'enc_pred_boxes': enc_topk_bboxes,}
...
# ##################################
# 
# Encoder Memory (multi-scale features)
#         │
#         ▼
#  [UMQS] Top-K Query Selection
#         │
#         ▼
#  Denoising Queries (optional, dn.py)
#         │
#         ▼
#  Initial Queries (target, ref_points_unact)
#         │
#         ▼
#  ┌─────────────────────────────────────────────┐
...
#  │ ┌─────────────────────────────────────────┐ │
#  │ │ Layer i:                                │ │
#  │ │   1. Self-Attn (queries ↔ queries)      │ │
#  │ │   2. Cross-Attn (MS DeformableAttn)     │ │
#  │ │      ↳ uses deformable_attention_core   │ │
#  │ │   3. FFN (linear → act → linear)        │ │
#  │ │   + Residual + Norm at each step        │ │
#  │ │                                         │ │
#  │ │   Detection Heads (per-layer):          │ │
#  │ │     - dec_score_head[i] (cls logits)    │ │
#  │ │     - dec_bbox_head[i]  (bbox offsets)  │ │
#  │ │                                         │ │
#  │ │   Iterative Refinement:                 │ │
#  │ │     ref_points ← sigmoid(bbox + inv_sig)│ │
#  │ └─────────────────────────────────────────┘ │
#  │                                             │
#  └─────────────────────────────────────────────┘
#         │
...
#    - dec_out_logits [B, num_queries, num_classes]
#    - dec_out_bboxes [B, num_queries, 4]
