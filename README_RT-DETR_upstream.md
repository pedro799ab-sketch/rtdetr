...
- Use the provided `.vscode/launch.json` configuration to "Run main.py" with an integrated terminal and `--device mps` (or `cpu`), or "Attach to debugpy" on port 5678 if remote debugging.

Steps:

1. Open the project in VS Code.
2. Select the `Python: Run main.py` configuration and press F5 to start with breakpoints.
3. Or run the script with `--debug` to enable DEBUG logs and automatic post-mortem pdb on unhandled exceptions:

```bash
python main.py --debug --device mps
```

Remote debugging inside Docker (debugpy):

1. Ensure the container is running and that port `5678` is available to the host (add `"5678:5678"` to `docker-compose.yml` `ports:` if needed).
2. Start the program under debugpy with `--wait-for-client` inside the container, e.g.:

```bash
# from repo root
./docker_debug.sh -- --device cpu
```

3. In VS Code, use the "Attach to debugpy (localhost:5678)" configuration to attach to the process and set breakpoints.

Tips:
- Use `--device mps` on Apple Silicon (M1/M2/M4) to use PyTorch MPS if available.
- The `--debug` flag enables verbose logs (DEBUG) and will try to drop into a post-mortem debugger on exceptions when run interactively.
