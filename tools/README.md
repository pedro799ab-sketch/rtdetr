...
```bash
   docker compose up --build -d # I don´t think that it is always necessary
   docker exec -it <your_container_name> bash

   # Export to ONNX
   python tools/export_onnx.py \
      -c  configs/rtdetrv2/rtdetrv2_r50vd_6x_coco.yml \
      -r  rtdetrv2_r50vd_6x_coco_ema.pth \
      --check
  
   # Convert to TensorRT
   bash tools/onnx2trt.sh model.onnx
...
# check memory assigned to the container (result is in bytes)
docker inspect 72ad8e118af2 --format='{{.HostConfig.ShmSize}}'
   # result default: 67108864
   # result after modifying docker-compose.yml: 17179869184

!!! need to re-build the container !!!
